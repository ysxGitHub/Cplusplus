...
否则，将请求安排给下一个空闲的服务器（服务器构成一个环，必要的话可能从第 0 
    个服务器开始继续找下一个空闲的服务器）。比方说，如果第 i 个服务器在忙，那么会查看第 (i+1) 
    个服务器，第 (i+2) 个服务器等等。
给你一个 严格递增 的正整数数组 arrival ，表示第 i 个任务的到达时间，和另一个数组 load ，
    其中 load[i] 表示第 i 个请求的工作量（也就是服务器完成它所需要的时间）。你的任务是找到 
...
vector<int> busiestServers(int k, vector<int> &arrival, vector<int> &load) {
    ...
    for (int i = 0; i < arrival.size(); i++) {
        ...
        // 注意这个 lower_bound 很重要 找到大于等于 j%k 空闲的服务器
        auto p = available.lower_bound(i % k);
        // 没有大于等于 j%k 空闲的服务器，从头开始分配
