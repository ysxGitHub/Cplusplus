/*
有效 IP 地址 正好由四个整数（每个整数位于 0 到 255 之间组成，且不能含有前导 0），
    整数之间用 '.' 分隔。

例如："0.1.2.201" 和 "192.168.1.1" 是 有效 IP 地址，但是 "0.011.255.245"、
    "192.168.1.312" 和 "192.168@1.1" 是 无效 IP 地址。
给定一个只包含数字的字符串 s ，用以表示一个 IP 地址，返回所有可能的有效 IP 地址，
    这些地址可以通过在 s 中插入 '.' 来形成。你 不能 重新排序或删除 s 中的任何数字。
    你可以按 任何 顺序返回答案。
*/
#include <iostream>
#include <vector>
#include <string>
using namespace std;

void backtrace(string&s, vector<string>&ans, string&path, int level, int idx, int cnt){
    if(level==4&&cnt==s.size()){
        if(path.size()&&path.back()=='.') path.pop_back();
        ans.push_back(path);
        return;
    }
    for(int i=1; i<=3&&idx+i<=s.size(); i++){
        string tmp = s.substr(idx, i);
        if(tmp[0]=='0'&&tmp.size()!=1) continue;
        int num = stoi(tmp);
        if(num>=0&&num<=255){
            path += tmp;
            if(level!=3) path += '.';
            backtrace(s, ans, path, level+1, idx+i, cnt+i);
            for(int j=0; j<tmp.size(); j++){
                path.pop_back();
            }
            if(level!=3) path.pop_back();
        }
    }
}

vector<string> restoreIpAddresses(string s) {
    int n = s.size();
    if (n>12) return {};
    string path;
    vector<string> ans;
    backtrace(s, ans, path, 0, 0, 0);
    return ans;
}